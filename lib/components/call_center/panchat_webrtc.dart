...
const apiRTCUrl = 'https://video.pancake.vn:4000/api';
const socketRTCUrl = 'wss://video.pancake.vn:4000/socket/websocket';
...
class PanchatWebRTC {
  ...
  Future<void> createAndSendOffer(offerData) async {
    ...
    try {
      ...
      //   if (await transceiver.getCurrentDirection() == TransceiverDirection.SendRecv
      //     ||(Platform.isMacOS && await transceiver.getDirection() == TransceiverDirection.SendRecv)) {
  ...
  Future<void> onAnswer(answer) async {
    ...
    try {
      ...
      await this
          .connection!
          .setRemoteDescription(description).then((value) => print("Set remote description success"))
          .onError((error, stackTrace) =>
              print("$error $stackTrace"));
    } catch (e, track) {
      print(e.toString() + "\n" + track.toString());
  ...
  Future<void> addTransceiversIfNeeded(Map<String, int> serverTracks) async {
    await this.connection!.getTransceivers();
    List recvTranceivers = [];
    // await Future.forEach<RTCRtpTransceiver>(tranceivers, (tranceiver) async {
    //   if ((Platform.isMacOS && (await tranceiver.getDirection()) == TransceiverDirection.RecvOnly)
    //     || Platform.isWindows && (await tranceiver.getCurrentDirection()) == TransceiverDirection.RecvOnly ){
