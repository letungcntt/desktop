...
class DirectMessage with ChangeNotifier {
  ...
  onChangeProfileFriend(data){
    _data = _data.map((e) {
      int index = e.user.indexWhere((element) => element["user_id"] == data["user_id"]); 
      if (index != -1) e.user[index] = {
        ...(e.user[index] as Map),
        ...data
      };
      return e;
  ...
  setSelectedDM(DirectModel dm, token, {bool isCreate = false}) async {
    ...
    if (indexConv != -1) dm = _data[indexConv];
      ...
      // them luon vao dataDMMessages voi khoi taoj conversationKey = GroupKey();
...
// };
    ...
    try {
      ...
      if (res["data"] != null ){
        ...
        if (resDM["conversation_id"] != null){
          await getInfoDirectMessage(token, resDM["conversation_id"], foreLoad: true);
        }
    ...
    try {
      ...
      if (responseData["success"] == false) {
        ...
        if (indexCurrent == -1 && Utils.checkedTypeEmpty(currentId)) {
          ...
          // dataDMMessages = dataDMMessages.where((element) => element["conversation_id"] != currentId).toList();
  ...
  Future removeDirectDeleted(String token) async {
    ...
    // toi da mac 1 sai lam khi da luu hoi thoai chua duowc tao vafo trong hive
    ...
    // Gio t phai lam gi??????????????????????????????????????????
    // doan code trong try duoi day de xu ly dieu do
    // no se lay het conversation_id cuar nguoi dung sau do se xoa nhung hoi thoai bi loi tren
    // trong tuong lai doanj nay se dc bo
    // --------------------------------------------------------------------------------------------------------------------------------
    ...
    try {
      ...
      for (var i = 0; i < needDelete.length; i++){
        ...
        dataDMMessages = dataDMMessages.where((element) => element["conversation_id"] != needDelete[i]).toList();
    ...
    try {
      ...
      for (var i = 0; i < directRemotes.length; i++) {
        ...
        dataDMMessages =  dataDMMessages.where((ele) => ele["conversation_id"] != convId).toList();
  ...
  saveDataToProvider(List dataResults, bool isReset) async {
    ...
    await direct.putAll(
      Map.fromIterable(getListCreated(_data), key: (v) => v.id, value: (v) => v)
    );
  }

    List<DirectModel> getListCreated(List<DirectModel> data){
    return data.map((e) {
      var c = getCurrentDataDMMessage(e.id);
...
//
  Future<bool> getInfoDirectMessage(String token, String conversationId, {bool foreLoad = false }) async {
    try {
      var indexD = _data.indexWhere((element) => element.id == conversationId);
      if (indexD != -1 && !foreLoad) return true;
      ...
      dataDMMessages = Map.fromIterable((dataDMMessages + dataResults.map((result) => result["dataMessageConversation"] as Map).toList()), key: (v) => v["conversation_id"], value: (v) => v as Map).values.toList();
      return true;

    } catch (e, t) {
      print("getInfoDirectMessage: $t and $conversationId and $e");
      return false;
    }
    ...
    try {
      ...
      dataDMMessages = _data.map((d) {
        ...
        if (d.id == idDirectMessage) {
          return {
            ...defaultConversationMessageData,
            "statusConversation": dataDMMessages[index]["statusConversation"],
            "conversation_id": d.id,
    ...
    try {
      ...
      if (dataRes["success"]) {
        ...
        await MessageConversationServices.insertOrUpdateMessage({
          ...message,
          ...dataRes["data"], 'parent_id': message['parentId'], 'is_thread': message["isThread"], 'is_blur': false,
          "success": true,
          "sending": false
        });
        if (!Utils.checkedTypeEmpty(message["isThread"])) {
      ...
      if (!message["isSend"]) return;
      print("queueBeforSend $e, $message $t");
      message["is_blur"] = true;
      message["success"] = false;
      message["sending"] = false;
      insetMessageErrorToReSend(message);
    ...
    try {
      ...
      List resultUpload = await Future.wait(
        atts.where((element) => element["content_url"] == null && element["mime_type"] != "share").map((item) async{
          ...
          return Provider.of<Work>(context, listen: false).uploadImage(token, null, uploadFile, uploadFile["type"], (){}, isDirect: true);
        })
      );
      ...
      print("sendMessageWithImage ___ $e $t");
      message["success"] = false;
      message["is_blur"] = true;
      updateDirectMessage(message, false, false, true);
    }
...
// luw lai thong tin nay de gui lai(chi cong voi cac tin ko phai thread)
  createMessageUploadFail(message) async {
    try {
      ...
      if (indexConverstionCurrent != -1){
        ...
        message["attachments"] = await Future.wait(
          (message["attachments"] as List).map((e)async {
            List<int> bytes = e["file_data"]["path"];
            File f = await File("$newPath/${DateTime.now().microsecondsSinceEpoch.toString()}").writeAsBytes(bytes);
      ...
      print("createMessageUploadFail: $e, $t");
    }
  }
...
// khoi phuc 1 conversation ve mac dinh
  resetOneConversation(String conversationId){
    ...
    dataDMMessages[indexDataMessage] = {
      ...defaultConversationMessageData,
      "statusConversation": dataDMMessages[indexDataMessage]["statusConversation"],
      "conversation_id": conversationId,
...
// chi remove khi hoi thoai >= 3 nguoif
  ...
  List getMessageErrorSavedOnHive(idConv){
    ...
    List messageQueues = boxQueueMessage.values
      .where((ele) => ele["conversation_id"] == idConv && !Utils.checkedTypeEmpty(ele["parentId"]))
  ...
  checkReSendMessageError(token, {bool onlySupportDM = false}) async {
    try {
      if (sendErrorMessage) {
        await Future.delayed(Duration(seconds: 10));
        return checkReSendMessageError(token);
      }
  ...
  void leaveConversation(String conversationId, String token, String currentUserId) async {
    ...
    Dio().post(
      ...
      if(value.data["success"]) {
        ...
        dataDMMessages =  dataDMMessages.where((ele) => ele["conversation_id"] != conversationId).toList();
        var box = Hive.box("direct");
