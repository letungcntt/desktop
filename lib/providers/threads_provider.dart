...
class Threads extends ChangeNotifier {
  ...
  loadMoreThread(token, workspaceId, Function setLoadMore) async {
    final index = _dataThreads.indexWhere((e) => e["workspaceId"] == workspaceId);
  ...
  updateThread(token, data, type, context) async {
    ...
    if (type == "newMessage") {
      ...
      if (data["message_id"] == null && data["issue_id"] == null) {
        ...
        final indexThread = _dataThreads[index]["threads"].indexWhere((e) => e["id"] == data["message_id"] || e["id"] == data["issue_id"]);
  ...
  updateUnreadThreadIssue(workspaceId, channelId, issueId, token) async {
    final url = "${Utils.apiUrl}workspaces/$workspaceId/update_unread_thread?token=$token";
    ...
    if (workspaceId != null && channelId != null) {
      ...
      if (index != -1) {
        final indexThread = _dataThreads[index]["threads"].indexWhere((e) => e["issue_id"] == issueId);
        if (indexThread != -1) {
          _dataThreads[index]["threads"][indexThread]["mention_count"] = 0;
          _dataThreads[index]["threads"][indexThread]["unread"] = false;
          try {
            var response = await Dio().post(url, data: {"message_id": null, "channel_id": channelId, "issue_id": issueId});
            var dataRes = response.data;
            if (dataRes["success"]) {}
