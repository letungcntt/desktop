...
class Auth extends ChangeNotifier {
  ...
  Future<dynamic> signUp(String firstName, lastName, userName, email, password, confirmPassword, context) async {
  ...
  Future<void> connectSocket(userId, params, {required BuildContext contextToSync}) async {
    ...
    channel.on("dm_message", (data, _ref, _joinRef) {
      bool isInMessageView = (Provider.of<Workspaces>(context, listen: false).tab == 0) && (Provider.of<Auth>(context, listen: false).onFocusApp);
    ...
    channel.on("delete_message", (data, ref, joinRef) {
      ...
      Provider.of<Channels>(context, listen: false).deleteAttachmentChannel(data);
    });
    ...
    channel.on("revice_data_sync", (data, _f, _j)async{
      try {
        ...
        if (dataServer["success"]){
          ...
          if (jsonDecode(messageDeStr)["flow"] == "file") {
            MessageConversationServices.handleSyncViaFile(masterKey.toBase64(), jsonDecode(messageDeStr)["url_sync_file"]);
            return;
          }
          List dataToSave  = [];
    ...
    //update status notify & pin channel
    ...
    channel.on("update_unread_threads", (data, ref, joinRef) {
      Provider.of<Threads>(context, listen: false).updateThread(token, data, "update_unread_threads", context);
    });
  }
