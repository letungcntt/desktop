...
class Channels extends ChangeNotifier {
  ...
  Map _currentApp = {};
  Map _attachmentsChannel = {};
  List _stickerChannels = [];
  List _queueIssueSelected = [];

  List get data => _data;
  Map get currentChannel => _currentChannel;
  Map get currentApp => _currentApp;
  ...
  Map get attachmentsChannel => _attachmentsChannel;
  List get stickerChannels => _stickerChannels;
  List get queueIssueSelected => _queueIssueSelected;
  ...
  onChangeOpenIssue(Map? data) async {
    ...
    if(data != null) {
      _queueIssueSelected.add(data);
      _issueSelected = data;
    } else if(_queueIssueSelected.length > 0) {
      _queueIssueSelected.removeLast();
      _issueSelected = _queueIssueSelected.length > 0 ? _queueIssueSelected.last : null;
    }
  ...
  updateAttachmentsChannel(data) {
    if (_attachmentsChannel['id'] != data['channel_id']) return;

    List typeAttachments = ['mp4', 'mov', 'flv', 'avi', 'image', 'video'];
    final List dataImages = data['files'].where((e) => typeAttachments.contains(e['type'])).toList();
    final List dataFiles = data['files'].where((e) => !typeAttachments.contains(e['type'])).toList();

    List newDataFiles = _attachmentsChannel['files'].where((e) => e['message_id'] != data['files'][0]['message_id']).toList();
    List newDataImages = _attachmentsChannel['image_videos'].where((e) => e['message_id'] != data['files'][0]['message_id']).toList();

    _attachmentsChannel['count_files'] = data['count_files'];
    _attachmentsChannel['count_images'] = data['count_image_and_video'];

    if(dataImages.isNotEmpty) _attachmentsChannel['image_videos'] = dataImages + newDataImages; 
    if(dataFiles.isNotEmpty) _attachmentsChannel['files'] = dataFiles + newDataFiles;
    notifyListeners();
  }

  deleteAttachmentChannel(data) {
    if(_attachmentsChannel['files'] is List && _attachmentsChannel['image_videos'] is List) {
      int beforeCountFile = _attachmentsChannel['files'].length;
      int beforeCountImage = _attachmentsChannel['image_videos'].length;

      List files = _attachmentsChannel['files'].where((e) => e["message_id"] != data['message_id']).toList();
      List imageVideos = _attachmentsChannel['image_videos'].where((e) => e["message_id"] != data['message_id']).toList();

      _attachmentsChannel['count_files'] = _attachmentsChannel['count_files'] - (beforeCountFile - files.length);
      _attachmentsChannel['count_images'] = _attachmentsChannel['count_images'] - (beforeCountImage - imageVideos.length);
      _attachmentsChannel['files'] = files;
      _attachmentsChannel['image_videos'] = imageVideos;
  ...
  Future<dynamic> selectChannel(var token, workspaceId, channelId) async {
    ...
    try {
      ...
      if (responseData["success"] == true) {
        _attachmentsChannel = {
          'id': channelId,
          'count_files': responseData['count_files'],
          'count_images': responseData['count_image_and_video'],
          'files': responseData['data_files'],
          'image_videos': responseData['data_images']
        };
  ...
  Future<dynamic> changeChannelMemberInfo(String token, workspaceId, channelId, member, String type) async {
    ...
    try {
      ...
      if (responseData["success"] == true) {
        ...
        if(type == "changStatusNotify") {
          updateStatusNotify(channelId, _currentMember["status_notify"]);
        }
      } else {
        throw HttpException(responseData['message']);
      }
  ...
  uploadSticker(workspaceId, channelId, token, data) async{
    final url = Utils.apiUrl + 'workspaces/$workspaceId/channels/$channelId/create_sticker?token=$token';

    try {
      final response = await Dio().post(url, data: data);
      final responseData = response.data;
      if (responseData["success"]) {
        final int index = _stickerChannels.indexWhere((e) => e['id'] == channelId);
        if(index != -1) {
          _stickerChannels[index]['stickers'].add(responseData['data']);

          notifyListeners();
        }
        sl.get<Auth>().showAlertMessage("Thêm mới Sticker thành công", false);
      } else {
        sl.get<Auth>().showAlertMessage("Thêm mới Sticker thất bại", true);
      }
    } catch (e) {
      sl.get<Auth>().showAlertMessage("Thêm mới Sticker thất bại", true);
      print(e.toString());
  ...
  getDataSticker(workspaceId, channelId, token) async{
    final url = Utils.apiUrl + 'workspaces/$workspaceId/channels/$channelId/get_sticker?token=$token';
    ...
    try {
      ...
      if (responseData["success"]) {
        ...
        if(index != -1) {
          _stickerChannels[index]['stickers'] = responseData['data'];
        } else {
          _stickerChannels.add({
            'id': channelId,
            'stickers': responseData['data']
          });
        }
  ...
  getStickerChannel(channelId) {
    final int index = _stickerChannels.indexWhere((e) => e['id'] == channelId);

    if(index != -1) return _stickerChannels[index]['stickers'];
    return [];
  }
}
