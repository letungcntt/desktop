...
class Workspaces with ChangeNotifier {
  ...
  /* required data element _listWorkspaceMembers
    {
  ...
  */
  ...
  Future<dynamic> getListWorkspace(context, String token) async {
    ...
    var snapshot = await Hive.openBox("snapshotData_$currentUserId");
    ...
    try {
      ...
      if (responseData["success"] == true) {
        ...
        getMentions(token, _currentWorkspace["id"], false, (v) {setLoadMore = v;});
        ServiceZimbra.initAllAccount(_data.map((e) => e["id"]).toList());
      } else {
  ...
  Future<dynamic> getInfoWorkspace(String token, int workspaceId, context) async {
    ...
    try{
      ...
      if (response["success"] == true) {
        ...
        int index = _listWorkspaceMembers.indexWhere((ele) => ele['workspaceId'] == workspaceId);
        if(index == -1) {
          _listWorkspaceMembers.add({
            'workspaceId': workspaceId,
            'members': _members
          });
        } else {
          _listWorkspaceMembers[index]['members'] = _members;
        }
  ...
  getListUsers(workspaceId) {
    int index = _listWorkspaceMembers.indexWhere((ele) => ele['workspaceId'].toString() == workspaceId.toString());
    if(index != -1) return _listWorkspaceMembers[index]['members'];
    return [];
  }
  ...
  onSelectWorkspace(context, workspaceId) {
    ...
    getMentions(auth.token, workspaceId, false , (v) {setLoadMore = v;});
  ...
  Future<dynamic> transferOwner(String token, String userId, String password) async {
    final url = Utils.apiUrl + 'workspaces/${_currentWorkspace['id']}/transfer_owner?token=$token';
    try {
      final body = {
        "user_id": userId,
        "password": password
  ...
  Future<dynamic> changeAppIds(String token, workspaceId, appIds) async {
    final url = Utils.apiUrl + 'workspaces/$workspaceId/update_app_ids?token=$token';

    try {
      final response = await http.post(Uri.parse(url), headers: Utils.headers, body: json.encode({"app_ids": appIds}));
      final responseData = json.decode(response.body);

      if (responseData["success"] == true) {
        _currentWorkspace["app_ids"] = appIds;
        final index = _data.indexWhere((e) => e["id"].toString() == workspaceId.toString());
        _data[index]["app_ids"] = appIds;
      } else {
        throw HttpException(responseData['message']);
      }
  ...
  Future<dynamic> deleteWorkspace(String token, int workspaceId, context) async {
    ...
    try {
      ...
      if (responseData["success"] == true) {
        _selectedTab = 0;
        sl.get<Auth>().showAlertMessage("Xoá wrokspace thành công", false);
        notifyListeners();
      } else {
        throw HttpException(responseData['message']);
      }
  ...
  updateWorkspace(data){
    ...
    if (lastIndex >= 0){
      ...
      _data[lastIndex]["app_ids"] = data["app_ids"];
      _data[lastIndex]["timesheets_config"] = data["timesheets_config"];
    }
    if (_currentWorkspace["id"] == workspaceId ){
      _currentWorkspace["name"] = data["name"];
      _currentWorkspace["app_ids"] = data["app_ids"];
      _currentWorkspace["timesheets_config"] = data["timesheets_config"];
    }
    notifyListeners();
  }
  ...
  onSaveStatePinned(context, id, isShowChannel, isShowPinned, isShowAppAdded) async{
    final currentUser = Provider.of<User>(context, listen: false).currentUser;
    ...
    if (index > -1) {
      ...
      data[index]["isShowAppAdded"] = isShowAppAdded ?? data[index]["isShowAppAdded"];
    }
    box.put("data", data);
    // _data = data;
  }
  ...
  Future<dynamic> getMentions(String token, workspaceId, bool loadMore, Function setLoadMore ) async {
  ...
  updateUnreadMentionItem(token, workspaceId, channelId, mention) async {
    final url = Utils.apiUrl + 'workspaces/$workspaceId/channels/$channelId/unread_mention_item?token=$token';

    var indexW = _mentions.indexWhere((element) => "${element["workspace_id"]}" == "$workspaceId");
    if (indexW == -1) return;
    var indexM = _mentions[indexW]["data"].indexWhere((e) => e["id"] == mention["id"]);
    if (indexM == -1) return;
    Future.delayed(Duration(milliseconds: 2500), () {_mentions[indexW]["data"][indexM]["unread"] = false;});
    
    try {
      final response = await Dio().post(url, data: json.encode({'mention': mention}));
      final responseData = response.data;
  ...
  updateDeleteWorkspace(context, token, data) {
    ...
    _data = _data.where((e) => e["id"] != workspaceId).toList();
    notifyListeners();
  }
  ...
  leaveWorkspace(token, workspaceId, userId) async {
    ...
    try {
      ...
      if (res["success"] == true) {
        ...
        sl.get<Auth>().showAlertMessage("Thoát workspace thành công", false);
        if (index == -1) return;
        _data.removeAt(index);
        setTab(0);
      } else {
